...
fun IngresosScreen(
    ...
    var filtroCategoria by remember { mutableStateOf<CategoriaIngreso?>(null) }
    var filtroFecha by remember { mutableStateOf("") }
    var expandedFiltro by remember { mutableStateOf(false) }

    val ingresosFiltrados = ingresos.filter { ingreso ->
        (filtroCategoria == null || ingreso.categoria == filtroCategoria) &&
        (filtroFecha.isBlank() || ingreso.fecha.contains(filtroFecha))
    }
    val total = ingresosFiltrados.sumOf { it.monto }
        ...
        Column(
            ...
            Card(
                ...
                Row(
                    ...
                    Text("Total ingresos:", fontWeight = FontWeight.Bold)
            ...
            if (ingresosFiltrados.isEmpty()) {
                ...
                LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    items(ingresosFiltrados, key = { it.id }) { ingreso ->
                        IngresoItem(
                            ingreso = ingreso,
                            onEditar = { onEditar(ingreso.id) },
                            onEliminar = { onEliminar(ingreso.id) }
                        )
                    }
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Row(
                    ...
                    Text(
                        ingreso.categoria.label,
                        fontSize = 16.sp,
                        fontWeight = FontWeight.Medium
                    )
                    Text(
                        "$ %.2f".format(ingreso.monto),
                        fontSize = 16.sp,
                ...
                if (ingreso.descripcion.isNotBlank()) {
                    Text(
                        ingreso.descripcion,
                        fontSize = 12.sp,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
